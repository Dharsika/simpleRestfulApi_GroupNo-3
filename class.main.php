	...
	class main {
		...
		public function takecall(){
			//if(strtolower($this->action) == 'show'){
				$query = " SELECT * FROM students WHERE id='$this->id' LIMIT 1";
		...
		/*	else if
			//(strtolower($this->action) == 'del')
			{
				$query = $this->link->prepare("DELETE FROM students WHERE id= :id");
		...
		public function add_new_record() {
			$query = $this->link->prepare("INSERT INTO ass_submissions(ass_id, ass_ref, ass_size, id, std_fname, date, time, ass_status, std_batch) VALUES(?,?,?,?,?,?,?,?,?)");
			$values = array($this->ass_id, $this->ass_ref, $this->ass_size_value, $this->id, $this->std_fname, DATE, TIME, 1, $this->std_batch);
			$query->execute($values);
			$count = $query->rowCount();
			return $count;
